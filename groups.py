...
class Group():
    ...
    @staticmethod
    def X(x, mixture, compounds):
        return x / sum([compound.x(mixture) * len(compound.groups) for compound in compounds])

    def theta(self, x, mixture, compounds):
        numerator = self.X(x, mixture, compounds) * self.Q
        denominator = 0
        for compound in compounds:
            for group in compound.groups:
                denominator += group.X(compound.x(mixture), mixture, compounds) * group.Q
        return numerator / denominator

    def ln_gamma(self, mixture, compounds, T):
        sigma_1 = 0
        for compound in compounds:
            for group in compound.groups:
                sigma_1 += group.theta(compound.x(mixture), mixture, compounds) * group.psi(self.name, T)

        sigma_2 = 0
        for compound_i in compounds:
            for group_i in compound_i.groups:
                numerator = group_i.theta(compound_i.x(mixture), mixture, compounds) * self.psi(group_i.name, T)
                denominator = 0
                for compound_j in compounds:
                    for group_j in compound_j.groups:
                        denominator += group_j.theta(compound_j.x(mixture), mixture, compounds) * group_j.psi(group_i.name, T)
